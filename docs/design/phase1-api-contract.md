...
- 各軸の境界（2026-08-18 決定#25。判定に必要な値が片方でもnullの軸は **OK＋理由「判定材料なし」**）
  ...
  - `startDate`（2026-08-27変更）: マッチング計算月を基準に動的判定する。要員の開始可能月が未来なら
    同月案件はOK、案件が後ならWARNING、案件が先ならNG。開始可能月が到来済みの要員は待機相当とみなし、
    当月・翌月案件をOK、翌々月以降をWARNINGとする。要員側の「即日」は計算月開始として同じ待機相当ルールを使う。
    案件側の「即日」は、要員が当月以前・翌月開始ならOK、翌々月以降ならWARNINGとする。
    稼働ステータスやLLM抽出項目は追加せず、既存の`start_ym`・`start_text`から都度導出する
...
- **共通スキル前置フィルタ**: 評価対象ペアは共通スキル1件以上のものにSQL段で絞る
  ...
  末尾保証で従来どおり残る）
- **鮮度フィルタ**: マッチング評価対象のopen案件は「開始時期が過去でない（start_ym IS NULLまたは
  当月以降。ただしstart_textが「即日」の案件は計算月開始として対象）」かつ
  「元メール受信から1ヶ月以内（手動登録案件=元メールなしは常に対象）」に限定。
