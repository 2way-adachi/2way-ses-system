...
| # | 論点 | 補足 |
|---|------|------|
| 1 | **メニューの表示制御** | SES大項目を全管理ユーザーに表示するか、権限で出し分けるか |
| 2 | **権限情報のフロント保持** | 現在Lavenderはログインユーザーの権限情報をフロントで保持していない。初期表示の出し分けにはログインAPIのレスポンス拡張が前提になる |
| 3 | **リポジトリの役割** | 実装は`2way_repository`に行うため、`2way-ses-system`をドキュメント専用にするか |
| 4 | パス命名規約 | ケバブケースに統一するか（4章参照） |
| 5 | Excelアップロード・PDF出力の実装方式 | Lavenderに同種の機能が既にあるかの確認を含む |
| 6 | スキーマをまたぐ管理ユーザー参照 | 権限判定でLavender側の管理ユーザーを参照する場合の解決方法 |
