...
### 論点1: 初期表示の権限判定をどこで行うか

「スキルシート」を開いたときの初期表示を権限で出し分ける（3章）にあたり、
現在のLavenderは**ログインユーザーの権限情報をフロント側で保持していない**。
判定をどこで行うかを決める必要がある。

| 案 | 内容 | 影響範囲 |
|----|------|---------|
| A | ログインAPIのレスポンスに権限情報を追加し、フロントの状態に保持して判定する | **大**。認証まわりの共通部分を変更するため、SES以外の既存機能にも波及する |
| B | フロントは権限を保持せず、スキルシートを開いたときに専用APIが初期表示の種別を返す | **小**。SESの新規API内で完結する |
...
SESメニューを常時表示とする決定により、フロントが権限を知る必要があるのは
初期表示の判定1箇所のみとなった。そのために認証の共通部分を変更するのは
既存機能へのリスクに見合わないため、**B案を推奨する**。

判定をサーバー側に置けば、画面側を操作されても権限判定が迂回されない利点もある。
