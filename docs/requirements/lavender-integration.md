...
- 一般ユーザーの参照範囲（2026-08-17確定）: **自分のメンバー詳細（スキルシートタブ含む）／自分のスキルシートの編集・下書き／案件メール一覧・詳細（参照のみ）**。
  マッチング・提案・要員一覧・メール取込実行・案件編集は管理者のみ
- **勤怠提出元は本人には非表示**（管理者のみ参照・編集。API側で担保）
- 許可範囲（ロール×機能）は**一元管理**し、後から容易に切り替えられる実装とする
  （フロントのメニュー出し分けと、API側の許可リストを、それぞれ1か所に集約して定義する）
- 本人紐付けはメンバー（要員）にログインユーザーIDを持たせ、要員編集画面で設定する（2026-08-17決定）
