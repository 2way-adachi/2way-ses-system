...
- [ ] **E1. メニュー文言「自分のスキルシート」→「スキルシート」**（プレフィックス削除。
  一般ログインには本人のものしか出ないため冗長）
- [ ] **E2. 本人向けマッチングビュー（判断系非表示）**（thyme側**実装完了**2026-08-26・
...
- [ ] **E3. スキルシート更新リマインド** — 最終更新日が閾値超過なら編集への誘導表示
  （シート鮮度=マッチング精度の生命線。本人の更新動機づけ）
- [ ] **E4. スキルシートPDFの本人利用** — 一般ロールでもヘッダーPDF表示（印刷）を使えるように
  （客先面談前の自己確認用途）
