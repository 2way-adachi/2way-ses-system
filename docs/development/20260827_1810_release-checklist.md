...
## 7. メール取込の初期データ方針（2026-08-28 人間確定）

- **過去分のバックフィルはしない（自然充填方式）**。リリース時に定期取込を有効化するだけとし、
  鮮度窓1ヶ月が実流量で自然に埋まるのを待つ（リリース直後の母集団が薄いのは許容）
- 却下した代替案（記録）: 過去1ヶ月一括取込は直近実流量（約2,000通/日×30日≈6万通）に対し
  LLM処理が実測換算70時間級の単一runとなり中断リスク過大。期間指定バックフィル機能の追加も
  「初日からフル母集団」の要望が出るまで作らない
- 設定: 本番ymlは initial-since-days=1・since-margin-days=1（IT同値）・enabled=true+fixed-delay
  （周期は#0で確定）。IMAPの定期SEARCH+ヘッダ照合は軽量で15分周期でも共有サーバー負荷の懸念なし

## 既知の注意（リリース判断に影響しうる）

- G1バグ: **修正済み**（thyme 69d807d・2026-08-27。起動時に宙吊りrunを自動失敗クローズ）→
  **8/28追加修正2件（未コミット）**: ①dev DB共有下でローカル起動が他ホストの実行中runを誤クローズ
  →host列で同一ホスト限定に ②finishRunがerror_message未クリアで成功後も中断文言残置→明示クリアに。
  リリースjarにはこの2修正を含めること。host列ALTER（docs/ddl/alter-mail-import-runs-host-20260828.sql）も
  DDL手順（§1）に追加
- G2: **解消済み**（2026-08-28。Matchingテスト修正04d1a6f・テストJDK11運用是正）
- 提案管理一覧mockの「読み込み中...」バグ → **修正済み**（2026-08-28・Lavender未コミット。
  真因はメンバー詳細提案タブのloading解除漏れ）
- Y5（マッチングバッチ化）: 実装中（2026-08-28）。リリース構成に含める場合はmatching_resultsテーブルの
  DDL追加とbackfill API実行（初期計算）を§1・§6に組み込むこと
