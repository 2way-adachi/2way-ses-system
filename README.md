...
「案件メール一覧」「要員メール一覧」「メンバー一覧」「マッチング一覧」「提案管理一覧」を配置する
（2026-08-17再整理。スキルシートはメンバー詳細内のタブとする）。
...
- メンバー一覧・詳細（info／スキルシート／提案／マッチングのタブ）
- スキルシートの一覧、詳細、新規作成、編集（下書き自動保存・復元含む）
- 経歴、スキル、資格、自己PRの管理
- PDF表示（印刷用HTML＋ブラウザ印刷。職務経歴書レイアウト）
- ~~既存Excelからの初期取り込み~~ → **中止**（2026-08-16決定）

### マッチング・提案管理【基盤実装済み・構成は要件とすり合わせ中】

- 要員と案件のマッチング（実装はスキルマスタ方式）
- マッチング一覧・承認/見送り（要件。実装は未追従）
- 提案管理（ステータス管理・メンバー詳細タブとの連携）
...
実装進行中（2026-08-17時点。いずれも**未コミット**）。

| 領域 | 状況 |
|------|------|
| スキルシート管理 | 実装済み・実DB結合確認済み（独立メニュー構成。タブ集約へは未追従） |
| 要員・案件・提案・マッチング | 基盤実装済み・実API接続（最新のメニュー構成・承認フローへは未追従） |
| 案件・要員メール取り込み | 画面モックのみ。取り込みバッチ・API未実装 |

実装と要件の対応・乖離は [実装現状の記録](docs/development/implementation-status.md) を参照。
画面モック（gh-pages）は多数の要件変更に未追従（[モック追従状況](docs/development/mock-sync-status.md)）。
